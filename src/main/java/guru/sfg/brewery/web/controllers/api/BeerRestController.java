...
import guru.sfg.brewery.security.perms.custom.annotations.beer.BeerDeletePermission;
...
public class BeerRestController {
    ...
    public ResponseEntity<BeerDto> getBeerById(@PathVariable("beerId") UUID beerId,
                                               @RequestParam(value =
                                                       "showInventoryOnHand", required
                                                       = false) Boolean showInventoryOnHand) {
        log.debug("Get Request for BeerId: " + beerId);
        if (showInventoryOnHand == null) {
            showInventoryOnHand = false;
        }
        return new ResponseEntity<>(beerService.findBeerById(beerId, showInventoryOnHand),
                HttpStatus.OK);
    }
    ...
    public ResponseEntity saveNewBeer(@Valid @RequestBody BeerDto beerDto) {
        ...
        httpHeaders
                .add("Location", "/api/v1/beer_service/" + savedDto.getId().toString());
